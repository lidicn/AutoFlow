...
SUBFLOWS: dict[str, SubflowSpec] = {
    "demo_notify": SubflowSpec(
        ...
        "的 link_out 编译路径。entry_link_id 为占位 id，需用户自备真实下游入口"
        "（或改用 bark_push / anysearch 等教学导入子流程）。",
        params={
        ...
        notes=(
            "【示例】参数形状与历史 tts_speak 完全一致，以便既有测试/golden fixture 经机械重命名"
            "即可继续作为 link_out 编译路径的回归基线。生产请改用 imported 子流程或教学导入。"
            "room 取值：default / 客厅 / 房间 / 卧室 / 书房 / 卫生间 / 主卧室 / 主卧室浴室 / 全屋。"
        ),
    ),

    # ── Bark 推送（iPhone 通知）────────────────────────────────────────────
    # subflow_id 指向 NR 上手动创建的 Bark 子流程；env 内置 BARK_SERVER/BARK_KEY（密钥不进 git，
    # 由部署时从 os.environ 注入）。param_style=flat → 入参平铺到 msg.title/msg.body/msg.bark_*。
    "bark_push": SubflowSpec(
        ...
        notes=(
            f"subflow_id={BARK_SUBFLOW_ID} 指向 NR 上手动创建的 Bark 子流程，其 env 内置 BARK_SERVER/BARK_KEY。"
    ...
    # subflow_id 已部署于 NR（Task #272，2026-07-21）。
    "history_state_at": SubflowSpec(
        ...
        notes=(
            ...
            f"subflow_id={HISTORY_STATE_AT_SUBFLOW_ID}（已部署于 NR，Task #272）。"
        ),
    ),
    ...
    "history_occurred": SubflowSpec(
        ...
        notes=(
            ...
            f"subflow_id={HISTORY_OCCURRED_SUBFLOW_ID}（已部署于 NR，Task #272）。"
        ),
    ),
    ...
    "history_duration": SubflowSpec(
        ...
        notes=(
            ...
            f"subflow_id={HISTORY_DURATION_SUBFLOW_ID}（已部署于 NR，Task #272）。"
        ),
    ),
    ...
    "history_aggregate": SubflowSpec(
        ...
        notes=(
            ...
            f"subflow_id={HISTORY_AGGREGATE_SUBFLOW_ID}（已部署于 NR，Task #272）。"
        ),
    ),
