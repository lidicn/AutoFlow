...
class Gateway:
    ...
    def dsl_help(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "grammar": {
                ...
                "时间段": "时间段: [<可选星期限定>] HH:MM-HH:MM   星期限定直接写中文词（工作日/周末/每天/周一..周日），**不要加方括号或竖线**：正确 `时间段: 工作日 20:00-23:00`、`时间段: 08:00-20:00`；错误 `时间段: [工作日|周末] 08:00-20:00`。也支持后缀写法 `时间段: 20:00-23:00 周末`。仅在时段内才继续执行缩进块。见 examples.工作日时间段",
                "延时": "延时: <数字> 秒",
                "并行": "并行:\n  动作: ...\n  动作: ...",
            },
            "constraints": [
    ...
    def deploy_proposal(self, pid: str, agent_id: str = "human",
        ...
        # ── Tab 组织模式：per_flow（默认）/ single_tab / mixed（P4）──
    ...
    def undeploy(self, flow_id: str, force: bool = False) -> Dict[str, Any]:
        ...
        # Tab 组织模式：single_tab/mixed 模式下读取对应 tab，否则读取 flow_id 对应的 tab
        from . import tab_organizer as tab_org
        ...
        # fail-atomic：NR 侧删除/更新失败时，不清账本，保持注册表与 NR 实际状态一致。
