...
每2分钟触发 (inject, repeat=120s)
    ↓
HTTP 健康检查 (GET http://<HA_HOST>:8080/api/health)
    ↓
...
健康检查失败
    ↓
exec: adb connect <HA_HOST>:5555 && adb shell am start -n com.example.arcfaceandroid/.TvMainActivity
```
...
- TV Cam 健康检查：http://<HA_HOST>:8080/api/health
