...
| # | 项 | 内容 | 验收门 | 前置 |
|---|---|---|---|---|
| 4 | **memory-worker 联动消费点** | 竞技场战报/agent 画像自动落 memory-worker；agent 经 ACP 查"我上轮学到什么"。网关侧 `delegate_to_memory_worker` 通道已存在，只缺竞技场消费点 | agent 能跨轮查到自己的历史经验，且**有真实读取行为**（不做只写不读） | ACP WebUI 前端落地 |
