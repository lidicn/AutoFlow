...
| **v2.3.0** | 实体解析决策智能 + 可信闭环 | 共用 | 实体解析决策层（device 归并/集成优选/弱信号降权/遥测）；快照/回滚稳定化；Arena 转正回归台；防假绿四件套+幽灵实体固化 | 进行中（#13/#14/#15 实体解析决策层已落地 2026-09-16；#16/#18 已落地 2026-09-16/17；#17 受 B1 独立 staging NR 阻塞） |
...
| # | 项 | 内容 | 验收门 |
|---|---|---|---|
| 10 | ✅ **引导式 DSL 编写**（commit e674ba1+8a00acd，2026-09-17 部署 NAS prod） | 对话/表单引导产出 DSL；编译后即 verify_flow 可视化（不暴露 NR 细节） | ✅ 小白按引导产出 1 条可验证 DSL |
