...
| **F-ACP-KEY（未决，跨项目契约）** | **两端 ACP 工具 schema 字段名不一致：本端 `input_schema`（snake）vs memory-agent `inputSchema`（camel，同 MCP 惯例）** | v2.0.12-2 收口 A20/A27 时顺带发现。两端**互不读取**对方 schema（memory-agent 的 `acp_routes` 只取 `t["name"]`；本端 `acp_client` 只做 delegate），故**当前无实际故障**，属潜伏不一致；一旦有 ACP 客户端按 MCP 惯例读 schema 就会踩空 | v2.0.12-2 实现时 grep 双侧源码实证 | **待裁决**：建议统一为 `inputSchema`（对齐 memory-agent + MCP 惯例）。改动是跨项目 wire 契约，需与 memory-agent 同步版本，故本轮只登记不改。改法：`"input_schema"` → `"inputSchema"` 一处 + 守卫锁死 |
