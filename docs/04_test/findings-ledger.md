...
| **F-P2-01（P1，已修，待部署）** | **DSL 帮助文档与编译器不一致 → 系统性误导所有写 DSL 的 agent** | `gateway.py:2203`（`dsl_help()` 语法声明）与 `skills/autoflow.md:93` 均写 `时间段: [工作日\|周末] HH:MM-HH:MM`，**编译器不认方括号**（`_parse_time_range` 只接受中文词前缀/后缀）。而同一份 `dsl_help()` 的 `examples.工作日时间段` 写的是正确形式 `时间段: 工作日 20:00-23:00` —— **语法声明与示例自相矛盾**。P2 A/B 两臂 + R8 团队**均踩中**：实测 `'[工作日\|周末] 08:00-20:00'` → `C_TIMERANGE_FORMAT` 拒绝 | P2 A/B（2026-09-11）两臂同题同错 + `_parse_time_range` 离线直测 | **已修**：`gateway.py:2203` 改为显式正确/错误对照（「星期限定直接写中文词，不加方括号」）；`skills/autoflow.md:93` 同步。全仓 grep 确认**无同类漂移**（仅此一处是真伪语法）。**需部署**（skills 需单独 scp，见 deploy skill §3.5） |
