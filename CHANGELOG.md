...
- 【新功能】内置 3 个示例模板：人体感应开灯、门窗开启通知、温度自动控制。
- 【新功能】WebUI 新增「模板库」页面：分类筛选、关键词搜索、模板详情、变量渲染、一键复制 DSL。
- 【新功能】模板变量语法：{{变量名}} 或 {{变量名|默认值}}，渲染时自动替换。
...
## 1.4.9 (2026-09-04)
- 【易用性改进】授权码管理页面顶部增加「授权码使用方法」说明卡片，6 步详细说明从创建到 Agent 使用的完整流程。
- 【易用性改进】创建授权码弹窗顶部增加简要说明，解释授权码的作用和使用方式。
- 【易用性改进】授权码创建成功弹窗增加「下一步：告知 Agent」指引，示范如何在对话中告诉 Agent 使用授权码。

## 1.4.8 (2026-09-04)
- 【Bug 修复】修复 /api/nr/tabs 节点数统计全部为 0 的问题。根因：Node-RED flows 是扁平结构，tab 节点本身不含 nodes 数组，普通节点通过 z 字段指向所属 tab。之前错误地读取 f.get("nodes")，永远为空。改为按 z 字段统计每个 tab 的节点数。
- 【新功能】授权码支持多 tab 绑定：创建授权码时可勾选多个目标 tab，Agent 只能在这些 tab 内部署/修改，不会越界到其他 tab。
- 【前端改进】创建授权码弹窗的「目标 tab」从下拉单选改为多选勾选框，增加「绑定到指定 tab」总开关，未勾选时走 per_flow 模式。
- 【安全增强】validate_token 增加 target_tab 越界检查：Agent 调用 MCP 时传入的 target_tab 如果不在授权码允许的列表中，直接拒绝并返回明确错误。
- 【兼容处理】target_tabs 与旧字段 target_tab 双向兼容：旧授权码只有 target_tab 时自动转为单元素列表；新授权码同时存储两个字段。

## 1.4.7 (2026-09-04)
- 【关键修复】在线更新 remote 恢复逻辑：之前 fetch 失败时 remote 可能没有被恢复，导致 origin 永久指向失效镜像，后续所有更新都失败。改用 try-finally 确保无论 fetch 成功还是失败，都恢复原 remote。
- 【健壮性改进】fetch 增加 60 秒超时，避免网络问题时无限等待（之前无超时，可能挂起）。
- 【诊断改进】fetch 失败时显示 git 的详细错误信息（stderr 前 200 字符），方便用户定位是网络问题还是镜像失效。
- 【镜像扩展】在线更新镜像选项从 3 个增加到 6 个：新增 kkgithub、gitmirror 镜像，并增加「自定义镜像」输入框，用户可输入任意镜像 URL。
- 【提示改进】镜像选择提示文案更新为"国内网络建议选镜像；若全部失败可手动 SCP 离线更新"。
...
- 【说明】用户反馈"豆包管家"授权码被意外吊销：经核实，第一个授权码创建后 2 分钟被吊销（用户自行重新创建），第二个授权码在 wb2 测试收尾时段（07:12）被批量吊销脚本误删。wb2 测试脚本应只吊销测试用授权码，不应吊销用户创建的授权码。
