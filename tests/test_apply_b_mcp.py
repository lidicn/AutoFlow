...
参照 tests/test_apply_mcp.py 模式，覆盖：
  - 工具进 _DEPLOY_KNIVES（普通身份 tools/list 不可见）
  - 专家身份调用 → 参数正确透传到 gateway.apply_state_from_debug（agent_id 由身份注入）
  - 普通身份调用 → 被拒（mode=normal 守卫）
...
def test_b_expert_passthrough(monkeypatch):
    fake = _FakeGw()
...
def test_b_normal_rejected(monkeypatch):
